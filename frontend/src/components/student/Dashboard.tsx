...
const Dashboard: React.FC<DashboardProps> = ({ userName, studId }) => {
  ...
  const fetchOverview = async (retries = retryCount) => {
    ...
    try {
      ...
      const [studentRes, assignmentsRes, attendanceRes] = await Promise.all([
        axios.get(`${import.meta.env.VITE_API_URL}/api/student/${studId}`, {
      ...
      setOverview({
        enrolledCourses: studentRes.data.enrolledCourses?.length || 0,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        <p>Track your courses, assignments, and progress at Woldia University.</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-blue-100 p-6 rounded-lg">
          <h3 className="text-lg font-semibold">Enrolled Courses</h3>
          <p className="text-3xl font-bold">{overview.enrolledCourses}</p>
        </div>
        <div className="bg-green-100 p-6 rounded-lg">
          <h3 className="text-lg font-semibold">Upcoming Assignments</h3>
          <p className="text-3xl font-bold">{overview.upcomingAssignments}</p>
        </div>
...
const Dashboard: React.FC<DashboardProps> = ({ userName, studId }) => {
  ...
  const fetchOverview = async (retries = retryCount) => {
    ...
    setError(null);
    try {
      console.log(`Fetching student overview data for ${studId}, attempt ${3 - retries}`);
      const token = localStorage.getItem('token');
      const [studentRes, assignmentsRes, attendanceRes] = await Promise.all([
        axios.get(`${import.meta.env.VITE_API_URL}/api/student/${studId}`, {
          headers: { Authorization: `Bearer ${token}` },
      ...
      if (
        !Array.isArray(studentRes.data.enrolledCourses) ||
      ...
      setOverview({
        enrolledCourses: studentRes.data.enrolledCourses.length,
        upcomingAssignments: assignmentsRes.data.filter(
          (assignment) => new Date(assignment.dueDate) > new Date()
        ).length,
        attendanceRate: attendanceRes.data.length
          ? Math.round(
              (attendanceRes.data.filter((record) => record.present).length /
                attendanceRes.data.length) *
                100
            )
          : 0,
